...
void HandleSqlScript(const std::string &script,
...

    {
        ...
        hybridse::plan::PlanAPI::CreateSyntaxTreeFromScript(script, parser_trees, &node_manager, sql_status);
        ...
        switch (node->GetType()) {
            ...
            default: {
                status.msg =
                    "Fail to execute script with unSuppurt type" + hybridse::node::NameOfSqlNodeType(node->GetType());
