...
bool Planner::CreateQueryPlan(const node::QueryNode *root, PlanNode **plan_tree, Status &status) {
    if (nullptr == root) {
    ...
    switch (root->query_type_) {
        case node::kQuerySelect:
            if (!CreateSelectQueryPlan(dynamic_cast<const node::SelectQueryNode *>(root), plan_tree, status)) {
                return false;
            }
            break;
        case node::kQueryUnion:
            if (!CreateUnionQueryPlan(dynamic_cast<const node::UnionQueryNode *>(root), plan_tree, status)) {
                return false;
            }
            break;
        default: {
            status.msg =
                "can not create query plan node with invalid query type " + node::QueryTypeName(root->query_type_);
            status.code = common::kPlanError;
            return false;
        }
    }
...
bool Planner::CreateSelectQueryPlan(const node::SelectQueryNode *root, PlanNode **plan_tree, Status &status) {
    ...
    for (node::SqlNode *node : table_ref_list) {
        ...
        if (!CreateTableReferencePlanNode(dynamic_cast<node::TableRefNode *>(node), &table_ref_plan, status)) {
            return false;
        }
    ...
    // select target_list
    if (nullptr == root->GetSelectList() || root->GetSelectList()->GetList().empty()) {
        status.msg =
    ...
    if (nullptr != root->GetWindowList() && !root->GetWindowList()->IsEmpty()) {
        for (auto node : root->GetWindowList()->GetList()) {
            ...
            if (windows.find(w->GetName()) != windows.cend()) {
                status.msg = "fail to resolve window, window name duplicate: " + w->GetName();
                status.code = common::kPlanError;
                return false;
            }
...
bool Planner::CreateUnionQueryPlan(const node::UnionQueryNode *root, PlanNode **plan_tree, Status &status) {
    if (nullptr == root) {
        status.msg = "can not create query plan node with null query node";
        status.code = common::kPlanError;
        LOG(WARNING) << status;
    ...
    if (!CreateQueryPlan(root->left_, &left_plan, status)) {
        status.msg = "can not create union query plan left query: " + status.str();
        status.code = common::kPlanError;
        LOG(WARNING) << status;
        return false;
    }
    if (!CreateQueryPlan(root->right_, &right_plan, status)) {
        status.msg = "can not create union query plan right query: " + status.str();
        status.code = common::kPlanError;
        LOG(WARNING) << status;
        return false;
    }
...
bool Planner::CheckWindowFrame(const node::WindowDefNode *w_ptr, base::Status &status) {
...
bool Planner::CreateWindowPlanNode(const node::WindowDefNode *w_ptr, node::WindowPlanNode *w_node_ptr,
...
bool Planner::CreateCreateTablePlan(const node::SqlNode *root, node::PlanNode **output,
                                    Status &status) {  // NOLINT (runtime/references)
...
bool Planner::ValidatePrimaryPath(node::PlanNode *node, node::PlanNode **output, base::Status &status) {
    if (nullptr == node) {
...
int SimplePlanner::CreatePlanTree(const NodePointVector &parser_trees, PlanNodeList &plan_trees,
                                  Status &status) {  // NOLINT (runtime/references)
    ...
    for (auto parser_tree : parser_trees) {
        switch (parser_tree->GetType()) {
            case node::kQuery: {
                ...
                if (!CreateQueryPlan(dynamic_cast<node::QueryNode *>(parser_tree), &query_plan, status)) {
                    return status.code;
                }
            ...
            case node::kCreateSpStmt: {
                ...
                if (CreatePlanTree(create_sp_tree->GetInnerNodeList(), inner_plan_node_list, status) !=
                    common::StatusCode::kOk) {
                    return status.code;
                }
                if (!CreateCreateProcedurePlan(parser_tree, inner_plan_node_list, &create_sp_plan, status)) {
                    return status.code;
                }
            ...
            default: {
                status.msg = "can not handle tree type " + node::NameOfSqlNodeType(parser_tree->GetType());
                status.code = common::kPlanError;
                LOG(WARNING) << status;
                return status.code;
            }
 ...
 * @param status
 */
bool Planner::CreateFuncDefPlan(const SqlNode *root, node::PlanNode **output,
                                Status &status) {  // NOLINT (runtime/references)
    if (nullptr == root) {
...
bool Planner::CreateInsertPlan(const node::SqlNode *root, node::PlanNode **output,
                               Status &status) {  // NOLINT (runtime/references)
    if (nullptr == root) {
...
bool Planner::CreateCreateProcedurePlan(const node::SqlNode *root, const PlanNodeList &inner_plan_node_list,
...
bool Planner::CreateTableReferencePlanNode(const node::TableRefNode *root, node::PlanNode **output, Status &status) {
    ...
    switch (root->ref_type_) {
        ...
        default: {
            status.msg =
                "fail to create table reference node, unrecognized type " + node::NameOfSqlNodeType(root->GetType());
            status.code = common::kPlanError;
            LOG(WARNING) << status;
            return false;
        }
...
bool Planner::MergeProjectMap(const std::map<const node::WindowDefNode *, node::ProjectListNode *> &map,
                              std::map<const node::WindowDefNode *, node::ProjectListNode *> *output, Status &status) {
    if (map.empty()) {
...
bool Planner::TransformTableDef(const std::string &table_name, const NodePointVector &column_desc_list,
                                type::TableDef *table,
                                Status &status) {  // NOLINT (runtime/references)
