...
void InitCases(std::string yaml_path, std::vector<SqlCase>& cases) {  // NOLINT
    if (!SqlCase::CreateSqlCasesFromYaml(
            ...
            std::vector<std::string>({"physical-plan-unsupport", "logical-plan-unsupport", "parser-unsupport"}))) {
        FAIL();
    }
}
...
INSTANTIATE_TEST_CASE_P(SqlWherePlan, TransformTest, testing::ValuesIn(InitCases("cases/plan/where_query.yaml")));

INSTANTIATE_TEST_CASE_P(SqlGroupPlan, TransformTest, testing::ValuesIn(InitCases("cases/plan/group_query.yaml")));

INSTANTIATE_TEST_CASE_P(SqlHavingPlan, TransformTest, testing::ValuesIn(InitCases("cases/plan/having_query.yaml")));

INSTANTIATE_TEST_CASE_P(SqlOrderPlan, TransformTest, testing::ValuesIn(InitCases("cases/plan/order_query.yaml")));

INSTANTIATE_TEST_CASE_P(SqlJoinPlan, TransformTest, testing::ValuesIn(InitCases("cases/plan/join_query.yaml")));

INSTANTIATE_TEST_CASE_P(SqlDistinctPlan, TransformTest, testing::ValuesIn(InitCases("cases/plan/distinct_query.yaml")));

INSTANTIATE_TEST_CASE_P(SqlSubQueryPlan, TransformTest, testing::ValuesIn(InitCases("cases/plan/sub_query.yaml")));
