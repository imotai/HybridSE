...
void AddFunc(const std::string& fn, ::hybridse::node::NodeManager* manager, ::llvm::Module* m) {
    if (fn.empty()) {
        return;
    }
    ...
    for (node::SqlNode* node : trees) {
        ...
        bool ok = fn_ir_builder.Build(dynamic_cast<node::FnNodeFnDef*>(node), &func, status);
        ASSERT_TRUE(ok);
    }
}
