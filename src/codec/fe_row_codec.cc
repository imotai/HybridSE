...
std::string RowView::GetStringUnsafe(uint32_t idx) {
    ...
    const char* val;
    uint32_t length;
    v1::GetStrFieldUnsafe(row_, field_offset, next_str_field_offset, str_field_start_offset_, str_addr_length_, &val,
                          &length);
...
int32_t RowView::GetValue(const int8_t* row, uint32_t idx, const char** val, uint32_t* length) const {
    ...
    return v1::GetStrFieldUnsafe(row, field_offset, next_str_field_offset, str_field_start_offset_, GetAddrLength(size),
                                 val, length);
}
...
int32_t RowView::GetString(uint32_t idx, const char** val, uint32_t* length) {
    ...
    return v1::GetStrFieldUnsafe(row_, field_offset, next_str_field_offset, str_field_start_offset_, str_addr_length_,
                                 val, length);
}
