...
Status UdfLibrary::RegisterFromFile(const std::string& path_str) {
    ...
    Status status;
    CHECK_TRUE(0 == parser.parse(script, parser_trees, node_manager(), status), kCodegenError,
               "Fail to parse script:", status.str());
    CHECK_TRUE(0 == planer.CreatePlanTree(parser_trees, plan_trees, status), kCodegenError,
               "Fail to create sql plan: ", status.str());
