...
class StringColumnImpl : public ColumnImpl<StringRef> {
 public:
    ...
    const StringRef GetFieldUnsafe(const Row &row) const override {
        ...
        StringRef value;
        const char *buffer;
        v1::GetStrFieldUnsafe(row.buf(row_idx_), str_field_offset_, next_str_field_offset_, str_start_offset_,
    ...
    void GetField(const Row &row, StringRef *res, bool *is_null) const override {
        ...
        if (buf == nullptr || v1::IsNullAt(buf, col_idx_)) {
            ...
            *is_null = false;
            ...
            StringRef value;
            const char *buffer;
            v1::GetStrFieldUnsafe(buf, str_field_offset_, next_str_field_offset_, str_start_offset_, addr_space,
                                  &buffer, &(value.size_));
            value.data_ = buffer;
            *res = value;
        }
